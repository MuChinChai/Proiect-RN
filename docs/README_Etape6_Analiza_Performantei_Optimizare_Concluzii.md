...
- [ ] **Model antrenat** salvat în `models/trained_model.nnet`
...
| **Reimprospatare** | Buton actionabil | Automat, la incarcarea imaginii | Viteza de lucru imbunatatita |
...
1. **Model înlocuit:** `models/trained_model.h5` → `models/optimized_model.h5`
   - Îmbunătățire: Accuracy +4%, F1 +2%
   - Motivație: rapiditate

2. **State Machine actualizat:**
   - Threshold modificat: [valoare veche] → [valoare nouă]
   - Stare scoasa: [idle] - [asteapta apasarea butonului refresh pentru a recunoaste; asteapta apasare buton stop, schimbare fisier]
   - Tranziție modificată: [de la incarcare poza se trece direct la detectie si recunoastere]
...
4. **Pipeline end-to-end re-testat:**
   ...
   - Timp total: [120] ms (vs [200] ms în Etapa 5)
```

### Diagrama State Machine Actualizată (dacă s-au făcut modificări)

diagrama actualizată în `docs/state_machine_v2.png`, diferențe: eficientizare, o stare inutila scoasa, productivitate imbunatatita
...
**Clasa cu cea mai bună performanță:** [Peste balon]
- Precision: [90]%
- Recall: [Y]%
- Explicație: [forma si trasaturi distincte]

**Clasa cu cea mai slabă performanță:** [Koi]
- Precision: [70]%
- Recall: [Y]%
- Explicație: [confuzie cu alta clasa (ton), exemple similare]

**Confuzii principale:**
1. Clasa [A] confundată cu clasa [B] în [10]% din cazuri
   - Cauză: [forma similara]
   - Impact industrial: [recunoastere gresita, incurcare procese ulterioare]
...
1. **Arhitectură:** [variație asezare neuroni pe straturi, variatie numar neuroni]
...
- Arhitectură: [80 neuroni primul strat, 40 neuroni al doilea strat]
- Learning rate: [valoare] cu [scheduler]
- Batch size: [valoare]
- Regularizare: [Dropout/L2/altele]
- Augmentări: [lista]
- Epoci: [20000] (early stopping la epoca [X])
...
**Obiective atinse:**
- [x] Model RN funcțional cu accuracy [80]% pe test set
...
- [x] [Accuracy sub target pentru clasa Koi]
- [x] [Documentație partial completă pe toate etapele]
**Obiective neatinse:**
- [x] [State machine finalizat]
```
...
1. **Limitări date:**
   - [Dataset dezechilibrat - clasa 'defect_mare' are doar 8% din total]
   - [Date analizate doar dupa procesare si formare imagine ideala]

2. **Limitări model:**
   - [Specii de pesti cu forma similara]
